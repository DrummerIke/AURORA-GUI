...
<div class="grid">
<div class="card"><h2>Phone Intelligence</h2><p>Проверка публичных упоминаний номера и заявленных кандидатом данных. Каталоги звонков не считаются доказательством личности.</p><form method="post" action="/run/phone"><label>Номер телефона</label><input name="target" required maxlength="32" inputmode="tel" placeholder="+7 999 123-45-67"><label>Заявленное ФИО (необязательно)</label><input name="claimed_name" maxlength="160" autocomplete="off" placeholder="Иванов Иван Иванович"><label>Цель проверки</label><select name="purpose"><option value="employment_due_diligence">Проверка кандидата при найме</option><option value="self_check">Проверка собственных данных</option><option value="corporate_antifraud">Корпоративный антифрод</option></select><label class="consent"><input type="checkbox" name="consent" value="yes" required><span>Подтверждаю законное основание и согласие на обработку данных для указанной цели.</span></label><button type="submit">Проверить публичные сведения</button></form></div>
...
def _start(kind: str, target: str):
    ...
    purpose = request.form.get("purpose", "authorized_osint_check").strip()
    consent = request.form.get("consent") == "yes"
    claimed_name = request.form.get("claimed_name", "").strip()
    if kind == "phone" and not consent:
        return "Необходимо подтвердить законное основание и согласие", 400
