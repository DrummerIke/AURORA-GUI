...
@dataclass
class SearchCase:
    id:str; input:dict; connector_runs:list[dict]; entities:list[dict]; evidence:list[dict]; rejected_candidates:list[dict]; summary:dict; relationships:list[dict]=field(default_factory=list); purpose:str=""; consent:bool=False
...
def normalize_person(value:str)->str: return re.sub(r"[^а-яёa-z-]+"," ",value.casefold()).strip()
def identifier_in_context(identifier:str, text:str)->bool:
    if identifier.casefold() in text.casefold(): return True
    wanted=re.sub(r"\D","",identifier)
    present=re.sub(r"\D","",text)
    return len(wanted)>=10 and wanted[-10:] in present
...
class WebSearchConnector(BaseConnector):
    ...
    async def search(self,inp):
        terms=[inp['normalized']]
        if inp.get('type') == 'phone':
            digits=re.sub(r'\D','',inp['normalized'])
...
class CandidateFilter:
    def valid_person(self,value, evidence, input_value):
        ...
        if not any(identifier_in_context(input_value,e.get('excerpt','')+e.get('title','')) for e in evidence): return False,"identifier_not_in_context"
        return True,""
    def valid_email(self,value,evidence,input_value):
        if not EMAIL_RE.fullmatch(value): return False,"invalid_email"
        if not any(identifier_in_context(input_value,e.get('excerpt','')+e.get('title','')) for e in evidence): return False,"no_context_link"
        return True,""
class EntityResolver:
    def resolve(self, inp, runs):
        evidence=[e for r in runs for e in r.get('evidence',[])]; rejected=[]; filt=CandidateFilter(); input_value=inp['normalized']
        ...
        for ev in evidence:
            ...
            for nm in PERSON_RE.findall(ev.get('excerpt','')):
                ...
                else: by_person.setdefault(normalize_person(nm), {"value":nm,"evidence":[]})['evidence'].append(ev)
        claimed=normalize_person(inp.get('claimed_name',''))
        for key,item in by_person.items():
            ...
            if inds < 2 and not trusted:
                rejected.append(asdict(RejectedCandidate(item['value'],'person','insufficient_independent_confirmation',ents[0].get('source_url',''),ents[0].get('excerpt','')[:250]))); continue
            penalties=[] if not claimed or key==claimed else [25]
            score=ConfidenceScorer().score(True,sum(e.get('reliability',.5) for e in ents)/len(ents),inds,.95 if key==claimed else .8,penalties)
