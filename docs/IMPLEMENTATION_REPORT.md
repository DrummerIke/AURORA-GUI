...
- Phone flow теперь сохраняет `purpose`, `consent` и необязательное заявленное ФИО; российские варианты номера ищутся отдельно, а ФИО принимается только по независимому контексту.
- Добавлены `docs/TERMUX.md` и `scripts/install_termux.sh`.
...
- Текущие web evidence основаны на URL/title/snippet поискового провайдера; безопасная загрузка и архивирование полного первоисточника остаются следующим этапом.
- Проверка GitHub releases/licenses из среды сборки завершилась сетевым предупреждением: proxy вернул 403, поэтому версии не выдумывались и не фиксировались как проверенные.
...
## Улучшение итогового отчёта

Renderer теперь сначала формирует executive summary, затем карточки личности, компаний, контактов и профилей. Список URL перенесён в evidence-раздел, а статусы коннекторов и отброшенные кандидаты находятся в закрытых технических блоках. Добавлено извлечение юридических названий российских организаций (`ООО`, `ПАО`, `АО`, `АНО`, `НКО`, `ИП`) с теми же требованиями контекста и независимого подтверждения, что и для ФИО.

Обзор следующих законных интеграций и причины отказа от непрозрачных агрегаторов находятся в `docs/OSINT_ROADMAP.md`.

## Обновление Termux после этого PR
